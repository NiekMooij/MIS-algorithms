...
def transcritical_removal(H: nx.Graph(), tau_bif: float, comments: bool=False) -> list:
    ...
    # Keep running loop until bifurcation has occured
    while True and tau_bif * set_off_factor < 1.5:
        
        try:
            final_state = linalg.solve((tau_bif * set_off_factor) * H0 + np.identity(len(H0)), np.ones(len(H0)), assume_a='sym')
            
        except:
            nodes_to_remove = []
            break
        
        nodes_to_remove = [ [ node for node in H ][i] for i in range(len(H)) if final_state[i] < 1e-5 ]
        
        # If no bifurcation has occured, we increase the tau value.
        if len(nodes_to_remove) == 0:
            set_off_factor *= 1.0001
            if comments: print('Bifurcation did not occur. Setoff increased.')
...
def pitchfork_removal(H: nx.Graph(), tau_bif: float, comments: bool=False) -> list:
    ...
    initial_state_perturbed = [item if item < selection_threshold or item > 1-selection_threshold else item + random.uniform(0, perturbation) for item in initial_state]
    
    # Keep running loop until bifurcation has occured. This can happen because Newton methods is not specific enough
    while True:
        tau_f = tau_bif * setoff_forward_factor
        
        # Determine values after bifurcatioon        
        final_state = LV_integrate(H, tau_f, initial_state_perturbed, t_end = t_end)
        
        nodes_to_remove = [ [ node for node in H ][i] for i in range(len(H)) if final_state[i] < selection_threshold ]    
...
def continuation_iteration(G: nx.Graph(), comments: bool=False) -> list:
    ...
    while H.number_of_edges() > 0:
        ...
        subgraphs = [ H.subgraph(subgraph) for subgraph in list(nx.connected_components(H)) if len(H.subgraph(subgraph)) != 1 ]
        bifurcation_points = [ newtonmethod( F, tau_initial, tolerance_newton, subgraph ) for subgraph in subgraphs ]
        tau_bif = min( [ bifurcation_point[0] for bifurcation_point in bifurcation_points ] )
        pitchfork_flag = any([ bifurcation_point[1] for bifurcation_point in bifurcation_points if abs(bifurcation_point[0] - tau_bif) < 1e-5 ])
        ...
        if not pitchfork_flag:
            ...
            if comments: print('Bifurcation type: Transcritical')
           
        elif pitchfork_flag:
            """Pitchfork bifurcation"""
            nodes_to_remove = pitchfork_removal(H, tau_bif, comments)
            
            if comments: print('Bifurcation type: Pitchfork')
        ...
        if comments: print(f'Network size: {len(H)}\n')
